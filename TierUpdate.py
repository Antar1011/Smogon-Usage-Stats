...
#it assumes the current tiers are stored in a file called "formats-data.json,"
#which is exported from PS data. To generate the file youself, open a Javascript console and
#copy/paste this file: https://raw.githubusercontent.com/Zarel/Pokemon-Showdown/master/data/formats-data.js
...
#file, and save as formats-data.json, and you should be good to go!
#
...
def makeTable(table,name,newTiers,keyLookup):
...
def main():
	...
	for poke in formatsData:
		...
		if old == 'Illegal':
			continue
		elif old not in ['Uber','OU','BL','UU','BL2','RU','BL3','NU']:
			old = 'PU'
		curTiers[poke]=old
		newTiers[poke]='PU'
	...
	#...first month's...
	getUsage(str(sys.argv[1])+"/Stats/ou-1695.txt",0,1.0,usage) #OU
	getUsage(str(sys.argv[1])+"/Stats/uu-1630.txt",1,1.0,usage) #UU
	getUsage(str(sys.argv[1])+"/Stats/ru-1630.txt",2,1.0,usage) #RU
	getUsage(str(sys.argv[1])+"/Stats/nu-1630.txt",3,1.0,usage) #NU

	#...second month
	getUsage(str(sys.argv[2])+"/Stats/ou-1695.txt",0,3.0,usage)
	getUsage(str(sys.argv[2])+"/Stats/uu-1630.txt",1,3.0,usage)
	getUsage(str(sys.argv[2])+"/Stats/ru-1630.txt",2,3.0,usage)
	getUsage(str(sys.argv[2])+"/Stats/nu-1630.txt",3,3.0,usage)

	#...third month
	getUsage(str(sys.argv[3])+"/Stats/ou-1695.txt",0,20.0,usage)
	getUsage(str(sys.argv[3])+"/Stats/uu-1630.txt",1,20.0,usage)
	getUsage(str(sys.argv[3])+"/Stats/ru-1630.txt",2,20.0,usage)
	getUsage(str(sys.argv[3])+"/Stats/nu-1630.txt",3,20.0,usage)
	...
	makeTable(OU,"OU",newTiers,keyLookup)
	makeTable(UU,"UU",newTiers,keyLookup)
	makeTable(RU,"RU",newTiers,keyLookup)
	makeTable(NU,"NU",newTiers,keyLookup)
