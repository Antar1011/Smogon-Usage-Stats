...
#which is compiled by hand from the PS console. The file isn't a true json. Instead, it's
#three jsons concatenated (one on each line). The first line is BattleFormatsData, and the
#second is BattleFormats. So to generate the file youself, go onto PS (where the tiers are
#hopefully up-to-date), open a Javascript console, and type: JSON.stringify(BattleFormatsData).
#Paste that output to a text file, make a new line, then repeat with BattleFormats. Save as
#showdowntierstuff.json, and you should be good to go!
#
...
#So the syntax is: python TierUpdatev2.py Month1folder Month2folder Month3folder
