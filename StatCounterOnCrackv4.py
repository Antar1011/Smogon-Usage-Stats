...
for entry in species:
	...
	if trainerNextLine:		
		...
		trainer = entry[:i]
		j = string.find(entry,',',i+9)
		rating = float(entry[i+9:j])
		i = string.find(entry,',',j+7)
		bias = int(entry[j+7:i])
		j = string.find(entry,',',i+13)
		stalliness = float(entry[i+13:j])
		tags = entry[j+7:string.find(entry,')',j+7)].split(',')
