...
elite = []
ratings = csv.reader(open(sys.argv[2], 'rb'), delimiter=',')
ratings.next()
for line in ratings:
	if eval(line[3]) >= 1337:
		#print line[3]
...
for entry in range(0,len(species)):
	...
	found = False
	if trainerNextLine:
		trainer = string.lower(species[entry][0:len(species[entry])-1])
		...
		if stemp != "???\n":
			...
			if not found:
				...
				sys.exit()
if len(sys.argv)>3:
	pickle.dump(encounterMatrix,open(sys.argv[3],"w"))
